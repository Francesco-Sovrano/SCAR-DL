...
import misc.tfidf_lib as common_lib
...
class SentenceClassifier(ModelManager):
	...
	def prepare_tfidf(self):
		...
		spacy_lemmatized_documents = [self.nlp(common_lib.get_document_from_words_vector(doc)) for doc in lemmatized_documents]
		if self.with_stemmed_tfidf:
			stemmed_documents = [common_lib.get_stemmed_document_from_words_vector(doc) for doc in lemmatized_documents]
			if self.log:
				print('stemmed_documents', stemmed_documents)
		# Build tf-idf model and similarities
		dictionary, tfidf_model, tfidf_corpus_similarities = common_lib.build_tfidf(
	...
	def get_docvec_similarity(self, queries):
		query_embeddings = self.cached_embed(queries)
	...
	def get_combined_wordvec_similarity(self, lemmatized_query, combination_fn=np.average):
		spacy_lemmatized_query = self.nlp(common_lib.get_document_from_words_vector(lemmatized_query))
	...
	def get_query_similarity(self, query):
		...
		if self.with_stemmed_tfidf:
			stemmed_query = common_lib.get_stemmed_document_from_words_vector(lemmatized_query)
		if self.log:
		...
		similarity_dict['tfidf'] = common_lib.get_query_tfidf_similarity(stemmed_query if self.with_stemmed_tfidf else lemmatized_query, self.dictionary, self.tfidf_model, self.tfidf_corpus_similarities)
		...
		if self.use_docvec:
			...
			if self.with_topic_scaling or not self.use_tf_model:
			...
			if self.with_topic_scaling:
				...
				similarity_dict['corpus'] = common_lib.get_corpus_similarity(similarity_dict['combined_wordvec'])
		...
		similarity_dict['weighted'] = common_lib.get_weighted_similarity(
		...
		if self.with_centered_similarity:
			similarity_dict['weighted'] = common_lib.get_centered_similarity(similarity_dict['weighted'])
